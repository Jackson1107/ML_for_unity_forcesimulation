...
class BacktestingEngine(object):
    ...
    def __loadArbTicksFromLocalCache(self, filename):
        ...
        # cache文件
        cacheFile = u'{0}/{1}.pickle'.format(cacheFolder, filename)
    ...
    def realtimeCalculate(self):
        ...
        for result in resultDict:
            ...
            self.pnlList.append(result.pnl)                             # ------------------pnlList
    ...
    def savingDailyData(self, d, c, m, commission, benchmark=0):
        ...
        dict_['shortPos'] = json.dumps(short_list, indent=4)
        dict_['longMoney'] = long_pos_occupy_money
        dict_['shortMoney'] = short_pos_occupy_money
        dict_['occupyMoney'] = max(long_pos_occupy_money, short_pos_occupy_money)
        dict_['occupyRate'] = dict_['occupyMoney'] / dict_['capital']
        dict_['commission'] = commission
        dict_['benchmark'] = benchmark
        self.dailyList.append(dict_)

        # 更新每日浮动净值
        self.netCapital = dict_['net']

        # 更新最大初次持仓浮盈净值
        if dict_['net'] > self.maxNetCapital:
            self.maxNetCapital = dict_['net']
            self.maxNetCapital_time = dict_['date']
        drawdown_rate = round((float(self.maxNetCapital - dict_['net'])*100)/m, 4)
